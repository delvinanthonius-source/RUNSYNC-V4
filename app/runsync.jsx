...
export default function RunSync(){
  ...
  const [foodLog,setFoodLog]=useState([
    FOOD_DB[0],FOOD_DB[11],FOOD_DB[30],FOOD_DB[38]
  ]);
  const [hydration,setHydration]=useState(3);
  const [weightHistory,setWeightHistory]=useState([
    {date:"1 Jan",weight:67},{date:"8 Jan",weight:66.5},{date:"15 Jan",weight:66},{date:"22 Jan",weight:65.3},{date:"29 Jan",weight:65},{date:"5 Feb",weight:64.5},{date:"Hari ini",weight:64.2}
  ]);
  const [mealPlan,setMealPlan]=useState(null);const [streak]=useState(7);
  const extra={foodLog,setFoodLog,hydration,setHydration,weightHistory,setWeightHistory,mealPlan,setMealPlan,streak};

  // ══ CAPTURE MODE: ?s=landing → render single screen for html.to.design ══
